...
def main(usr_args):
    ...
    # kept at the stock value so a phrasing is drawn from the same pool a stock run draws from.
    test_num = 100

    model = get_model(usr_args)
    episode_results = eval_policy(task_name,
                                  TASK_ENV,
                                  args,
                                  model,
                                  seeds,
    ...
    with open(file_path, "w") as file:
        ...
        for row in episode_results:
            file.write(f"{row['seed']}\t{row['success']}\t{row['instruction']}\n")
        successes = sum(row["success"] for row in episode_results)
        file.write(f"\nSuccess rate: {successes}/{len(episode_results)}\n")
    ...
    for now_id, now_seed in enumerate(seeds):
        ...
        if expert_check:
            try:
                ...
                raise RuntimeError(
                    f"The expert script failed on seed {now_seed}, so this episode has no scene "
                    f"parameters to fill an instruction with. Stock eval_policy would move to the next "
                    f"seed; a given seed list cannot.") from e
            if not (TASK_ENV.plan_success and TASK_ENV.check_success()):
                print(f"\033[93mThe expert did not solve seed {now_seed}; evaluating it regardless.\033[0m")
        ...
        episode_results.append({"seed": now_seed, "success": succ, "instruction": instruction})
